...
async function loadData(){try{const [response,translations,summaries]=await Promise.all([fetch('data/research.json?v=20260922-app'),fetch('data/translations.json?v=20260922-app'),fetch('data/about_summaries.json?v=20260922-app')]);if(!response.ok||!translations.ok||!summaries.ok)throw Error(t('loadError'));state.records=await response.json();state.translations=await translations.json();state.aboutSummaries=await summaries.json();
