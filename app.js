...
const PAGE_SIZE=40;
const state={records:[],translations:{},aboutSummaries:{},lang:localStorage.getItem('camx-language')==='en'?'en':'ko',market:'전체',segment:'전체',scope:'all',query:'',shown:PAGE_SIZE,selected:null};
...
async function loadData(){try{const [response,translations,summaries]=await Promise.all([fetch('data/research.json?v=20260919-auto2'),fetch('data/translations.json?v=20260919-auto2'),fetch('data/about_summaries.json?v=20260919-auto2')]);if(!response.ok||!translations.ok||!summaries.ok)throw Error(t('loadError'));state.records=await response.json();state.translations=await translations.json();state.aboutSummaries=await summaries.json();
